import type { Sitemap, SitemapConfig } from "../config";
...
export class SiteMapBuilder {
  ...
  constructor(private config: SitemapConfig) {
    ...
    this.officialNameAssociations = new Map(
      Object.entries(this.config.pageTitles).map(([k, v]) => [
        onlyTrailingSlash(k),
        v,
      ]),
    );
  ...
  async addHTMLContent(filePath: string, folderPath: string) {
    ...
    const linkPath = path
      .join(
        this.basePath,
        this.getLinkPath(filePath.slice(0, -5), folderPath, ""),
      )
      .replace(/\\/g, "/");
    ...
    const starlightSidebar =
      content.match(
        /<ul[^>]*class=["'][^"']*\btop-level\b[^"']*["'][^>]*>/gs,
      )?.[0] ?? "";
    const sidebarLinkIdentifier =
      starlightSidebar.match(
        /class=["'][^"']*\b(astro-[^"'\s]*)[^"']*["']/s,
      )?.[1] ?? "";
    const starlightPagination =
      content.match(
        /<div[^>]*class=["'][^"']*\bpagination-links\b[^"']*["'][^>]*>/gs,
      )?.[0] ?? "";
    const paginationIdentifier =
      starlightPagination.match(
        /class=["'][^"']*\b(astro-[^"'\s]*)[^"']*["']/s,
      )?.[1] ?? "";
    ...
    for (const tag of content.match(/<a\b[^>]*?>.*?<\/a>?/gm) ?? []) {
      ...
      if (
        this.config.ignoreStarlightLinks &&
        paginationIdentifier &&
        classes.includes(paginationIdentifier)
      ) {
        continue;
      }
      ...
      if (link.length && !link.startsWith("#")) {
        ...
        if (link && text.length) {
          ...
          this.implicitNameAssociations.set(link, [
            ...(this.implicitNameAssociations.get(link) ?? []),
            text,
          ]);
        }
      }
    ...
    if (this.config.linkInclusionRules.length) {
      links = new Set(
        [...links].filter((link) =>
          firstMatchingPattern(link, this.config.linkInclusionRules, false),
        ),
      );
    }
  ...
  async addMDContent(filePath: string) {
    ...
    if (frontmatter.data) {
      ...
      if (frontmatter.data.sitemap?.pageTitle) {
        this.officialNameAssociations.set(
          linkPath,
          frontmatter.data.sitemap.pageTitle,
        );
      } else if (
        frontmatter.data.title &&
        !this.officialNameAssociations.has(linkPath)
      ) {
        this.officialNameAssociations.set(linkPath, frontmatter.data.title);
      }
    }

    const currentLinkRules = (
      frontmatter.data?.sitemap?.linkInclusionRules ?? []
    ).concat(this.config.linkInclusionRules);
    if (currentLinkRules.length) {
      links = new Set(
        [...links].filter((link) =>
          firstMatchingPattern(link, currentLinkRules, false),
        ),
      );
    }
  ...
  toSitemap(): Sitemap {
    return Object.fromEntries(
      Array.from(this.map.entries()).map(([_, entry]) => [
        entry.linkPath,
        {
          ...
          title:
            ...
            getMostCommonItem([
              ...(this.implicitNameAssociations.get(entry.linkPath) ?? []),
            ]) ??
  ...
  private resolveLink(current: string, link: string, links: Set<string>) {
    if (!link.startsWith("http")) {
      if (link.startsWith(".")) {
        ...
        this.basePath !== "" &&
        !trimSlashes(link).startsWith(this.basePath)
      ) {
        link = path.join(this.basePath, link);
      }
      link = slugifyPath(
        onlyTrailingSlash(link.split("#")[0]!).replace(/\\/g, "/"),
      );
      if (link !== current) {
        links.add(link);
        return link;
  ...
  private getLinkPath(
    ...
    relative_path =
      basePath === ""
        ...
        : path.join(basePath, relative_path).replace(/\\/g, "/");
